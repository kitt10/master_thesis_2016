...
"""
    ...
    This script prunes the trained net. It starts with the weights close to zero, using tanh instead of sigmoid.
...
def cut_synapses(net, level):
    """ cuts synapses """
    if level > 0:
        th_change = np.percentile([abs(synapse.get_weight()) for synapse in net.synapsesG], level)
    else:
        th_change = min([abs(synapse.get_weight()) for synapse in net.synapsesG])

    c = 0
    for synapse in net.synapsesG[:]:
        synapse.set_weight()
        if abs(synapse.weight) <= th_change:
            synapse.remove_self()
            c += 1
    n_to_remove = c
...
if __name__ == '__main__':
    ...
    destination = '../cache/pruned/'+args.net+'zero_p_'+args.name_appendix+'.net'
