...
It is a good manner to keep the data normed - mapped to $ [0.0, 1.0] $ interval. The default range of foot contact sensors is already set to $ [0.0, 1.0] $, so there is nothing to change. For the foot contact sensors, following approach is used to map the data. The bounds are defined by default sensors ranges (listed in \cref{tab:proprioceptors}).
